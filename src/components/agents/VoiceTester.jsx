    ...
    try {
      ...
      if (agent.knowledge_base_ids && agent.knowledge_base_ids.length > 0) {
        ...
        for (const kbId of agent.knowledge_base_ids) {
          try {
            const chunks = await base44.entities.KnowledgeChunk.filter({ knowledge_base_id: kbId });
            if (chunks?.length) {
              allText.push(...chunks.map(c => c.content));
            }
          } catch {}
        }
        // Limit context to ~8k chars for latency
        knowledgeContext = allText.join('\n\n').slice(0, 8000);
      }

      const guardrail = "Answer strictly using the provided Knowledge Base or site content. If the information is not present, say you don’t know and offer to follow up. Do not fabricate.";
