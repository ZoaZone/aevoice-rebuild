...
export default function WebhookTester({ phoneNumber }) {
  ...
  const testWebhook = async () => {
    ...
    try {
      // Use number_e164 if present, otherwise fallback to sip_address
      const toNumber = phoneNumber.number_e164 || phoneNumber.sip_address;
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-500">
            Use this URL in Twilio Console → Phone Numbers → {phoneNumber.number_e164 || phoneNumber.sip_address} → Voice Configuration
          </p>
        </div>
