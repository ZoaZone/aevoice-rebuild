 ...
 * Shared utilities for phone number processing and validation
 */
...
export function formatPhoneNumber(
  ...
  // Simple national format (can be improved with libphonenumber)
  ...
  // Default: return international format
  return normalized;
}

/**
 * Parse SIP address to extract phone number
 * @param sipAddress - SIP address (e.g., "sip:user@domain" or "user@domain")
 * @returns Phone number or null if not found
 */
export function parseSIPAddress(sipAddress: string): string | null {
  if (!sipAddress) {
    return null;
  }

  // Remove "sip:" prefix if present
  const cleaned = sipAddress.replace(/^sip:/i, "");

  // Extract user part before @
  const atIndex = cleaned.indexOf("@");
  if (atIndex > 0) {
    const userPart = cleaned.substring(0, atIndex);
